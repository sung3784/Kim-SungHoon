...
function modalTaraeOutput(resp) {
	...
	if (resp.user.saveFile != null) {
		mention += "<h3> <img class='ui avatar image' style='width:70px; height:50px;float:left; ' src='http://10.0.8.226:8888/sns/resources/images/FileRepo/"
				+ resp.user.saveFile + "' />";
	} else {
		mention += "<h3> <img class='ui avatar image' style='float:left; ' src='http://localhost:8888/sns/resources/images/defaultAvatar.png' />";
	}
	...
	$.each(resp.photo, function(index, item) {
		mention += "<img class='img_show_example' src='http://10.10.8.226:8888/sns/resources/images/FileRepo/" + item.saveImg + "'>";
