      ...
      $.ajax({
         ...
         <!-- ================================= userid ===================================================== -->          
               ...
               if (data.profilePic == null) {
                  tare += "<img src='http://localhost:8888/sns/resources/images/defaultAvatar.png' class='ui avatar image'>";
               }
               if (data.profilePic != null) {
                  tare += "<img src='http://10.10.8.226:8888/sns/resources/images/FileRepo/" + data.profilePic + "' class='ui avatar image'>";
               }
         ...
         <!-- ================================== content+photo+regDate ========================================== -->             
                ...
                var photo = data.saveFile;
                  ...
                  for(var k=0; k<saveFile.length; k++){
                     tare += "<img class='tarae_img' src='http://10.10.8.226:8888/sns/resources/images/FileRepo/"+saveFile[k]+"'>";
                    }
                }
		...
		<c:if test="${alarmList != null }">
	         <c:forEach varStatus="num" var="al" items="${alarmList}">
	            ...
	            <div class="ui feed" align="center">
	               <div class="event">
	                  <div class="label">
	                     <c:if test="${al.profilePic == null}">
	                        <img class="ui avatar image"
	                           src="http://localhost:8888/sns/resources/images/defaultAvatar.png">
	                     </c:if>
	                     <c:if test="${al.profilePic != null }">
	                        <img class="ui avatar image"
	                           src="http://10.10.8.226:8888/sns/resources/images/FileRepo/${al.profilePic }">
	                     </c:if>
	                  </div>
	                  <div class="content" id="feedContent${num.index}" onclick="ajax(${num.index})">
	                     ...
	                     <c:if test="${al.noticeType == 'like'}">
	               		 ...
	               		 <c:if test="${al.noticeType == 'unLike'}">
                           <a>${al.loginId}</a> Unlike feed &nbsp; ${al.regDate}
                         </c:if>
	               ...
	               <c:if test="${ al.noticeType != 'book'}">
	               <img style="width:50px;height:30px;" class="imgAlarm" src="http://10.10.8.226:8888/sns/resources/images/FileRepo/${photo}"/> 
	               </c:if>
