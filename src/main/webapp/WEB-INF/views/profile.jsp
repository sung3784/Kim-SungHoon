	...
	<script>
	   ...
	   function mentionWriteFunction(feedNum) {
		...
		function output(resp) {
		    ...
		    mention += "<div class='existed-inner ui padded segment'>";
		    ...
		    if(resp.user.saveFile != null) {
		       //mention += "<h3> <img class='mentionWriteProfile' style='width:70px; height:50px;float:left; ' src='./resources/images/FileRepo/"+resp.user.saveFile+"' />";
		    	mention += "<h3> <img class='mentionWriteProfile' style='width:70px; height:50px;float:left; ' src='http://10.10.8.226:8888/sns/resources/images/FileRepo/"+resp.user.saveFile+"' />";
		    } else {
								...
								<div class="item">
									<div class="ui horizontal list">
										<c:if test="${user.saveFile == null }">
											<div class="item">
												<img class="profile" src="http://localhost:8888/resources/images/defaultAvatar.png">
												<h2>${user.userId }</h2>
											</div>
											<div class="item">
									...
									<div class="ui horizontal list">
										<c:if test="${user.saveFile != null }">
											<div class="item">
												<img class="profile" src="http://10.10.8.226:8888/sns/resources/images/FileRepo/${user.saveFile }">
												<h2>${user.userId }</h2>
											</div>
											<div class="item">
									...
									<div class="item">
									   	<div class="feedUser">
									    	<c:if test="${feed.saveFile == null}">
												<img class="ui avatar image" src="http://localhost:8888/resources/images/defaultAvatar.png">
									    	</c:if>
									    	<c:if test="${feed.saveFile != null }">
							    		...
							    		<input type="hidden" id="originalFeedNum${num.index }" value="${feed.originalFeedNum }">
							    			...
							    			<c:if test="${feed.photoYN eq 'Y'}">
								    			<c:forTokens items="${feed.photoSaveFile }" delims="," var="photo">
													<img class="img_show_example" src="http://10.10.8.226:8888/sns/resources/images/FileRepo/${photo}" style="width: 100px; height: 100px;">
								    			</c:forTokens>
							    			</c:if>
			...
			<!-- ==============================================타래 modal ================================================= -->		    
				...
				<c:if test="${ps != null }">
					<c:forEach varStatus="num" var="cg" items="${ps }">
						...
						<input type ="hidden" id="number" value="number${num.index }">
					</c:forEach>
				</c:if>
