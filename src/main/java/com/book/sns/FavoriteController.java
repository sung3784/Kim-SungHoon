...
public class FavoriteController {
	...
	public @ResponseBody int favorite1(int feedNum, HttpSession session) {
		...
		alarm.setLoginId(id);
		Alarm unAlarm = new Alarm();
		unAlarm.setFkNum(feedNum);
		unAlarm.setNoticeType("unLike");
		unAlarm.setLoginId(id);
		...
		if (exist == 1) {
			...
			for (Relationship r : follow) {
				unAlarm.setOwnerId(r.getLoginId());
				mapper2.insertFavoriteAlarm(unAlarm);
			}
