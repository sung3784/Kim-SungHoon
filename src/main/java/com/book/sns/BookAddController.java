...
public class BookAddController {
	...
	public String bookAdd(String searchIsbn, Model model, @RequestParam(value = "page", defaultValue = "1") int page) {
		...
		try {
			...
			String apiURL = "https://dapi.kakao.com/v2/search/book?page=" + page
					+ "&target=title&target=isbn&target=publisher&query=" + keyword;
	...
	public String selfAdd(Book book, HttpSession session, MultipartFile thumb) {
		...
		System.out.println("섬네일" + thumb.getOriginalFilename());
		String userId = (String) session.getAttribute("userId");
		...
		if (thumb.getSize() == 0) {
			...
			String saveName = uuid + "_" + thumb.getOriginalFilename();
			book.setThumbnail("http://10.10.8.226:8888/sns/resources/images/FileRepo/" + saveName);
			//book.setThumbnail(saveName);
	...
	public String goBookAdd(Model model, String searchIsbn) {
		System.out.println("searchIsbn : '" + searchIsbn + "'");

		String authorization = "KakaoAK 62f3a956ce11a4820b9408378304d68d";
		String result = "";
		Book oneBook = new Book();

		String[] isbns = searchIsbn.split(" ");
		if (isbns.length > 1) {
			searchIsbn = isbns[1];
			System.out.println("isbn : " + isbns[1]);
		}
		...
		try {
			...
			for (int i = 0; i < documents.size(); i++) {
				...
				/*
				 * Iterator<String> iterator = authors.iterator(); while (iterator.hasNext()) {
				 * if(author.equals("")) { author = iterator.next(); } else { author += "," +
				 * iterator.next(); } //author = author + ", " + iterator.next(); }
				 */
